...
Album and track rows, the mini-player, Now Playing, and MediaSession notifications use authenticated cover artwork. The client requests server-generated 256 px derivatives for standard surfaces and 512 px derivatives for details/playback metadata, with explicit 15%-memory and 64 MiB disk cache limits. Artwork URLs remain token-free; dedicated OkHttp/Media3 loaders add authorization only for whitelisted same-origin cover routes, reject redirects, bound notification artwork to 8 MiB, downsample notification bitmaps, and drop idle cover sockets after 30 seconds. Android Auto's compact dashboard fetches `artworkUri` itself and cannot add a bearer token, so the playback service embeds a validated 256 px derivative (at most 1 MiB) on the current item and clears that URI from published Media3 metadata. Embed validation reads image bounds without allocating a notification bitmap. Compose keeps using the stored extras URL.
