...
Detail responses expose metadata and opaque related IDs, not indexed roots or source paths. Album collection and detail responses include `added_at`, the RFC 3339 creation time of the newest indexed track belonging to that album; Android uses it only to order the bounded **Recently added** Home section. Track metadata uses lowercase `flac` or `mp3` format values and may include duration in milliseconds, sample rate, bit depth when available, channel count, genre, date, disc/track positions, and related artist/album/cover IDs. Track detail additionally includes `album_track_count`, the indexed track count for that album; list and search responses omit it. There is no playlist or saved-queue HTTP API; the Android client stores one optional queue snapshot locally and re-resolves track IDs through `GET /api/v1/tracks/{id}`.

For snapshot synchronization only, Android retries transport failures and HTTP 408, 429, 500, 502, 503, and 504, with no more than three attempts total per request. Other HTTP failures are not retried. Search, artist detail, track detail, and Android Auto Albums/Artists/search remain direct network clients; album detail and Auto home shelves may read the active local snapshot.
