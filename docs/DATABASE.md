...
Android also has an independent app-private Room database, `library-cache.db`, for public catalog metadata. Its version 2 schema is exported under `android/app/schemas/` and contains:
...
- `cached_artists`, `cached_albums`, and `cached_tracks`: generation-scoped public API models plus stable server order; cached albums also store the newest member track's index-creation time for Home recency.
...
Room queries join catalog rows to `cache_state.active_generation`, so staging data is invisible. Library lists use Room `PagingSource` instances configured with a page size of 50; Home reads bounded recently added and per-process discovery selections, and album detail reads active cached album tracks. Migration 1→2 adds album recency without exposing the staging generation and marks the retained snapshot for one background refresh. Search, artist detail, track detail, and Android Auto search do not query this cache. Android Auto Albums, Artists, root Recent, and Discover use active snapshot queries. Opening a cached album or artist in Auto reads the same active album/artist tracks as phone album detail.
