...
7. Only the empty-cache bootstrap requests a network summary and bounded album shelf while the full snapshot is built. Search, artist detail, and track detail remain network-backed. Android Auto Albums and Artists read Room windows from the active snapshot (network only if those windows are empty); Auto search stays network-backed. Auto home shelves and cached album/artist children also read Room.
