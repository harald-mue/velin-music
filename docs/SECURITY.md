...
- Apply rate limiting and safe failure responses to login, pairing, and token-sensitive endpoints. Rate-limit keys use the direct peer IP without its source port, ignore untrusted forwarded headers, and retain a bounded number of client buckets.
...
- The Android playback service is exported as a Media3 `MediaLibraryService` so Android Auto can bind. Each new Media3 OkHttp data source reloads the current credentials from private Keystore-backed storage, adds authorization headers in memory, rejects non-stream paths and origins other than the current paired server, and disables redirects. Credential changes release the old phone controller and stop the stale service. Browse media IDs, metadata, and controller-provided media URIs never contain tokens. Android Auto browse grids fetch `iconUri` themselves and ignore embedded `artworkData` for those tiles, so Auto browse covers use an exported `content://com.haraldmue.velin.artwork/covers/{id}/{size}` provider: opaque cover IDs only, pairing required, Coil disk cache first, then an in-memory authenticated fetch stored in that cache. Now Playing still embeds a validated 256 px `artworkData` bitmap (1 MiB cap) and omits the public HTTP URI. Compose retains the token-free HTTP URL in extras.
- The optional device-local saved queue is a private JSON file of opaque track IDs and public title/artist strings. It must not contain tokens, URLs with credentials, or filesystem paths.
- Automatic playback resumption uses a separate bounded private JSON file keyed by a SHA-256 credential namespace. It contains only opaque IDs, public display/format/cover metadata, index/position, shuffle, and repeat—never tokens, authorization, stream URLs, artwork bytes, or filesystem paths. The checkpoint-clear custom Media3 command is advertised only to Velin's own package and checks the controller package again before deleting state.
