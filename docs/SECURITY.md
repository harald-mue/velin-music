...
- The Android playback service is exported as a Media3 `MediaLibraryService` so Android Auto can bind. Each new Media3 OkHttp data source reloads the current credentials from private Keystore-backed storage, adds authorization headers in memory, rejects non-stream paths and origins other than the current paired server, and disables redirects. Credential changes release the old phone controller and stop the stale service. Browse media IDs, metadata, artwork URLs, and controller-provided media URIs never contain tokens. Android Auto's compact dashboard cannot authenticate those cover URLs, so the service may embed only the current item's validated 256 px derivative (1 MiB cap) as Media3 `artworkData` and omit the public `artworkUri` from published metadata; Compose retains the token-free URL in extras.
