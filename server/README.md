...
The `internal/library` package implements validated root persistence, bounded FLAC/MP3 discovery and metadata parsing, artwork caching with bounded 128/256/512 px derivatives, startup garbage collection, and cancellable background prewarming after startup and successful scan work. One worker coalesces triggers, deterministically selects at most 1,024 referenced covers per pass, ensures their 256 and 512 px variants, and pauses 10 ms after each variant so one pass stays within the derivative-cache budget. The package also implements transactional scan reconciliation, interrupted-scan recovery, globally serialized sequential multi-root orchestration with throttled live counters, optional startup scanning, periodic full-library scheduling, paginated artist/album/track queries, and ranked FTS5 track search.
